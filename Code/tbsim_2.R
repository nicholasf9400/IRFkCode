##Turning Bands sim
# Skal kalde noget sim på linjen når det er klar.
...
tbsim <- function(b,n.line, grid.list){
  theta <- seq(0,pi,length.out=n.line)
  m <- length(b)
  d <- rep(0,m)
  for(i in 0:(m-1)){
...
n.line <- 100 #Number of linesim
h <- c(25)
b=c(110)
# 
l <- tbsim(b,n.line,grid.list)
